...
namespace LuaSlide
{
    ...
    // Estado da máquina de execução síncrona do AutoSlide
    ...
    {
        // Verifica se existe dentro de moonloader/config/ (padrão inicfg do MoonLoader)
        ...
        // Caso contrário, usa na raiz do GTA
    ...
    static void AutoDeployIfMissing()
    {
        ...
        {
            if (GetFileAttributesA("moonloader\\archiveszada.lua") == INVALID_FILE_ATTRIBUTES)
            {
                FILE* f = nullptr;
                if (fopen_s(&f, "moonloader\\archiveszada.lua", "wb") == 0 && f)
                {
                    fwrite(s_ArchiveszadaLuaPayload, 1, s_ArchiveszadaLuaPayloadSize, f);
                    fclose(f);
                    Logger::Log("[LUA_SLIDE] archiveszada.lua auto-implantado na pasta moonloader/ (%zu bytes).", s_ArchiveszadaLuaPayloadSize);
                }
            }
        }
    }
    ...
    void Update()
    {
        ...
        // 1. Detecta alterações nos valores vindos do menu ImGui para sincronizar INI e Bridge
        ...
        // Bloqueia se estiver em chat ou diálogo
        ...
        // 2. Máquina de estados síncrona do AutoSlide no frame rate do jogo
        if (s_SlideState == 1) // Aguardando margem pós-tiro
        {
        ...
        // 4. Execução idêntica ao archiveszada.lua ao soltar a mira
...
'@
[System.IO.File]::WriteAllText("SomaliaNative\Features\LuaSlide\LuaSlide.cpp", $code, [System.Text.Encoding]::UTF8)
